...
// ===== PARALLAX EFFECT ON HERO GLOW (Desktop) =====
...
window.addEventListener('scroll', () => {
  if (window.scrollY > 100) {
    tabs?.classList.add('sticky-shadow');
  } else {
    tabs?.classList.remove('sticky-shadow');
  }
});

// ===== ADD RIPPLE TO DYNAMICALLY CREATED ELEMENTS (CART ITEMS) =====
const originalUpdateCartUI = updateCartUI;
updateCartUI = function() {
  originalUpdateCartUI();
  document.querySelectorAll('.cart-item button').forEach(btn => btn.classList.add('ripple'));
};

// ===== SMOOTH SCROLL FOR ALL ANCHOR LINKS (including external) =====
document.querySelectorAll('a[href^="#"]').forEach(anchor => {
  anchor.addEventListener('click', function(e) {
    const href = this.getAttribute('href');
    if (href === '#' || href === '#deals') {
...
// Add keyframe for cart fab pulse
