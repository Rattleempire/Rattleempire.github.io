...
const categoryInfo = {
  ...
  blog: { title: 'Empire Blog', desc: 'Tips, trends, and stories from the Rattle Empire community.' }
};
...
if (auth) {
  auth.onAuthStateChanged(user => {
    if (authBtn) {
      authBtn.innerHTML = user ? `<i class="fas fa-user-check"></i> ${user.email?.split('@')[0] || 'User'}` : '<i class="fas fa-user"></i> Sign In';
    }
  });
}
...
if (authBtn) {
  authBtn.addEventListener('click', () => {
    if (!auth) {
      showToast('Firebase not configured – guest mode', 'info');
      return;
    }
    if (auth.currentUser) {
      auth.signOut();
...
// ===== RENDER PRODUCTS (handles blog posts specially) =====
let currentProducts = [];
...
function updateCategoryHeader(category) {
  const info = categoryInfo[category] || { title: category, desc: '' };
...
function updateCartUI() {
  ...
  cartItems.innerHTML = cart.map((item, index) => {
    ...
    return `
      <div class="cart-item">
        <div><strong>${item.name}</strong><br><small>UGX ${item.price.toLocaleString()}</small></div>
...
function loadAdminProducts() {
  ...
  container.innerHTML = all.map(p => `
    <div style="display:flex; justify-content:space-between; padding:8px; border-bottom:1px solid var(--border-light);">
      ...
      <button onclick="removeProduct(${p.id})" style="color:red;"><i class="fas fa-trash"></i></button>
    </div>
  `).join('');
}
...
['https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?w=400',
 'https://images.unsplash.com/photo-1593642632823-8f785ba67e45?w=400'].forEach(src => {
...
style.textContent = `
  @keyframes cartPulse { 0%,100%{transform:scale(1);} 50%{transform:scale(1.2);} }
  .cart-fab:active { animation: cartPulse 0.3s ease; }
`;
