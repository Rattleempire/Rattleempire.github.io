...
document.getElementById("loginBtn").addEventListener("click", () => openModal("Sign in", "Authentication will connect to the production backend in the next phase."));
document.getElementById("cartBtn").addEventListener("click", () => openModal("Your cart", cart.length ? `${cart.length} item(s) are saved in this browser.` : "Your cart is empty."));
document.getElementById("sellerBtn").addEventListener("click", () => openModal("Seller onboarding", "Verified-seller onboarding will be connected to the seller application and admin review system."));
document.getElementById("themeBtn").addEventListener("click", () => {
  document.body.classList.toggle("light");
  localStorage.setItem("rattle_theme", document.body.classList.contains("light") ? "light" : "dark");
});
if (localStorage.getItem("rattle_theme") === "light") document.body.classList.add("light");
updateCart();
render();
